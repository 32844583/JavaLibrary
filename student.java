...
public class student extends normaluser {
	...
	public void action() {
		...
		int select;
		do {
			select = JOptionPane.showOptionDialog(null, "學生" + super.getname() + "您好：", "一般使用者介面",
		...
		if (4 > t.length && t.length > 0) {
			...
			if (result == 0) {
				if (userbooks.size() >= this.lendup) {
					JOptionPane.showMessageDialog(null, "超過借閱數量","借閱系統",1);
				} else {
					String response = "";
					if (adbook.getsituation() == "unavailable") {
						...
						if (you_borrow == 1) {
							...
							JOptionPane.showMessageDialog(null, response,"借閱系統",1);
						} else {
						...
						userbook ub = new userbook(adbook.getname(), adbook.getpublish(), adbook.getauthor());
						JOptionPane.showMessageDialog(null, "借閱至" + ub.getretrievedate(),"借閱系統",1);
						userbooks.add(ub);
			...
			JOptionPane.showMessageDialog(null, "目前無書籍！","借閱系統",1);
		}
	}
	...
	public void watchinfo() {
		if (userbooks.size() > 0) {
			...
			comboBox.setSelectedIndex(0);
			myPanel.add(comboBox);
			String retrieve_list[] = { "取消借閱或預約", "不動作" };
				...
				for (userbook ub : userbooks) {
					if (ub.getname() == comboBox.getSelectedItem().toString()) {
						if (ub.getsituation() == "reserved") {
							cancel_reserved(ub);
							JOptionPane.showMessageDialog(null, "取消預約！！","預約系統",1);
						} else {
							retrieve(ub);
						}
						break;
